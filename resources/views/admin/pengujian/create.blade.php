...
<div class="max-w-4xl mx-auto">
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Tambah Pengujian Baru</h2>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <form action="{{ route('admin.pengujian.store') }}" method="POST">
                @csrf
                            ...
                            @foreach($jenisPengujian as $jenis)
                                <label class="flex items-center p-4 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer">
                                    <input type="checkbox" name="jenisPengujianIds[]" value="{{ $jenis->id }}"
                                           {{ in_array($jenis->id, old('jenisPengujianIds', [])) ? 'checked' : '' }}
