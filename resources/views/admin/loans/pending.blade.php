...
<div class="space-y-8">
    ...
    <div class="bg-gradient-to-br from-amber-600 to-amber-700 rounded-xl p-8 text-white shadow-xl border border-amber-500">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold mb-2">Peminjaman Pending</h1>
                <p class="text-amber-100 text-lg">Kelola permintaan peminjaman alat laboratorium yang menunggu persetujuan</p>
            </div>
            <div class="flex items-center space-x-3">
                <div class="bg-white text-amber-700 px-6 py-3 rounded-xl shadow-lg font-semibold">
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-8">
        ...
        <form method="GET" action="{{ route('admin.loans.pending') }}" class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($loans as $loan)
                                <tr class="hover:bg-amber-50 transition-colors duration-200">
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-mono font-semibold text-amber-600 bg-amber-50 px-3 py-1 rounded-lg border border-amber-200">
                                            {{ $loan->tracking_code }}
                                        </div>
